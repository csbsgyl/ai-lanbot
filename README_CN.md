...
已有实例更新时会先创建一致性本地数据备份，再替换受管源码并启动新版本。
部署完成时会打印 QQ 回调反代上游；同机反代使用 `http://127.0.0.1:5300`，
默认填写到 QQ 开放平台的地址为 `https://idc.csbsgyl.com/qq/callback`。
登录后台后可在 **设置 > IDC 查询 > QQ 回调** 直接复制该完整公网回调地址，
并查看机器人模式、回调接收、验证、重复投递和拒绝计数。
...
点击[仓库首页](https://github.com/csbsgyl/ai-lanbot)右上角 Star 和 Watch 按钮，获取最新动态。
...
感谢所有[贡献者](https://github.com/csbsgyl/ai-lanbot/graphs/contributors)对 LangBot 的帮助：
